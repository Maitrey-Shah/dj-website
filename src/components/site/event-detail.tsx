    ...
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-2xl flex-col items-center justify-center px-5 text-center">
        ...
        <p className="text-muted-foreground mt-4">
          This event may have been moved or sold out.
        </p>
        <Link
          to="/"
          hash="events"
          className="bg-heat text-primary-foreground mt-8 rounded-full px-8 py-4 text-xs font-bold tracking-[0.22em] uppercase"
        >
          See All Events
        </Link>
      </div>
  ...
  return (
    <div>
      ...
      <section className="relative min-h-[80svh] overflow-hidden">
        ...
        <div className="relative mx-auto flex min-h-[80svh] max-w-[1400px] flex-col justify-end px-5 pb-28 pt-36 sm:px-8 sm:pb-20">
          ...
          <div className="mt-8 hidden flex-col gap-3 sm:flex sm:flex-row">
            <TicketCta ticketUrl={e.ticketUrl} soldOut={soldOut} />
            <a
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <Reveal>
              ...
              {e.tickets.map((t) => (
                ...
                >
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <p className="font-display text-xl font-extrabold">{t.price}</p>
                  </div>
            ...
            <div className="mt-6 hidden lg:block">
              <TicketCta ticketUrl={e.ticketUrl} soldOut={soldOut} wide />
            </div>
          </Reveal>
        </aside>
      </div>
      ...
      <div className="bg-background/95 border-border/60 fixed inset-x-0 bottom-0 z-40 flex items-center gap-3 border-t px-4 py-3 backdrop-blur-xl sm:hidden">
        ...
        <TicketCta ticketUrl={e.ticketUrl} soldOut={soldOut} compact />
      </div>
...
function TicketCta({
  ...
  if (soldOut) {
    return (
      <span
        className={`border-border/70 text-muted-foreground ${wide ? "block w-full" : "inline-block"} cursor-not-allowed rounded-full border px-8 py-4 text-center text-xs font-bold tracking-[0.22em] uppercase`}
      >
        Sold Out
      </span>
    );
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className={`bg-heat text-primary-foreground ${wide ? "block w-full" : "inline-block"} ${compact ? "px-6 py-3" : "px-8 py-4"} rounded-full text-center text-xs font-bold tracking-[0.22em] uppercase transition-transform duration-300 hover:scale-[1.03]`}
    >
      Get Tickets
    </a>
  );
